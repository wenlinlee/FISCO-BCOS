...
public:
    ...
    //    template <class AccessorType>  // handler return isContinue
    //    void forEach(uint64_t eachBucketTimeLimit, size_t totalTxLimit,
    ...
    //
    //                return bucket->template forEach<AccessorType>(
    //                    [&bucketStartTime, eachBucketTimeLimit, &totalTxLimit,
    //                        handler = std::move(handler)](typename AccessorType::Ptr accessor) {
    //                        if (utcTime() - bucketStartTime >= eachBucketTimeLimit)
    //                        {
    //                            BCOS_LOG(DEBUG)
    //                                << LOG_KV("bucketStayTime:", utcTime() - bucketStartTime)
    //                                << LOG_KV("eachBucketTxsLimit:", eachBucketTimeLimit)
    //                                << LOG_KV("totalTxLimit:", totalTxLimit);
    //                            return false;
    //                        }
    //                        if (totalTxLimit <= 0)
